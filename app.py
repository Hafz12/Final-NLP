...
file_path = r'C:\Users\leosy\Downloads\Final NLP\anime-recommendation-system-master\model\myanimeweights.h5'
...
with open(r'C:\Users\leosy\Downloads\Final NLP\anime-recommendation-system-master\model\anime_encoder.pkl', 'rb') as file:
    anime_encoder = pickle.load(file)

with open(r'C:\Users\leosy\Downloads\Final NLP\anime-recommendation-system-master\model\user_encoder.pkl', 'rb') as file:
    user_encoder = pickle.load(file)

# Load the dataset for additional information
with open(r'C:\Users\leosy\Downloads\Final NLP\anime-recommendation-system-master\model\anime-dataset-2023.pkl', 'rb') as file:
    df_anime = pickle.load(file)
    
df_anime = df_anime.replace("UNKNOWN", "")
    
# Load the user ratings dataset
df=pd.read_csv(r'C:\Users\leosy\Downloads\Final NLP\anime-recommendation-system-master\model\users-score-2023.csv', low_memory=True)
...
def get_animes_by_genre(genre_name, n=10):
    try:
        # Filter animes that contain the specified genre
        genre_animes = df_anime[df_anime['Genres'].str.contains(genre_name, case=False, na=False)]
        
        if genre_animes.empty:
            print(f"No animes found for genre '{genre_name}'")
            return pd.DataFrame()
        
        # Sort by score and get top n animes
        genre_animes = genre_animes.sort_values(by='Score', ascending=False).head(n)
        
        recommended_animes = []
        for idx, anime in genre_animes.iterrows():
            anime_image_url = anime['Image URL']
            ...
            recommended_animes.append({"anime_image_url": anime_image_url, "anime_name": anime_name, "Genres": genre,
                                       "Synopsis": Synopsis, "English Name": english_name, "Native name": other_name, "Score": score,
                                       "Type": Type, "Aired": aired, "Premiered": premiered, "Episodes": episodes, "Status": status,
                                       "Studios": studios, "Source": source, "Rating": rating, "Rank": rank, "Favorites": favorites,
                                       "Duration": duration, "Episode Duration": episode_duration, "anime_id": anime_id})
        ...
        print(f"Error in genre-based filtering: {str(e)}")
        return pd.DataFrame()
...
def recommend():
    ...
    if recommendation_type == "user_based":
        ...
        if not genre_name:
            return render_template('index.html', error_message="Please enter a Genre name.", recommendation_type=recommendation_type)
        
        recommended_animes = get_animes_by_genre(genre_name, n=num_recommendations)
        if recommended_animes is None or recommended_animes.empty:
            message3 = "No animes found for genre '" + str(genre_name) + "'"
            return render_template('recommendations.html', message=message3, animes=None, recommendation_type=recommendation_type)
